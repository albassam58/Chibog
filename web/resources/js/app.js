...
Vue.mixin({
	methods: {
		async getVendorUrl() {
			let { data } = await axios.get('/v1/vendor-url');
			return data;
		},
		...
		updateItemLocalStorage(key, newVal, index = null) {
			// Parse any JSON previously stored in key
		    var existingEntries = JSON.parse(localStorage.getItem(key));

		    if (existingEntries && index !== null) {
		    	existingEntries[index] = newVal;
		    	localStorage.setItem(key, JSON.stringify(existingEntries));
		    }
		},
		...
		completeAddress(data) {
			...
			return vm.capitalize(`${ data.street }, ${ data.barangay }, ${ data.city}`);
		},
		scheduleDay(data) {
			let vm = this;
